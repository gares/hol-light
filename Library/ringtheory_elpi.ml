...
Hol_elpi.accumulate (__POS_OF__ "

pred is_ring o:preterm, o:pretype.
is_ring X (ptycon \"fun\" [_, B]) :- is_ring X B.

% :before \"infer:lam:knowledge\"
% infer.knowledge (varp _ _ as R) (ptycon \"ring\" A) [is_ring R A].

pred ring_op i:preterm.
ring_op {{:raw ring_0 ^R }} :- var R.
ring_op {{:raw ring_1 ^R }} :- var R.
ring_op {{:raw ring_neg ^R }} :- var R.
ring_op {{:raw ring_add ^R }} :- var R.
ring_op {{:raw ring_mul ^R }} :- var R.
ring_op {{:raw ring_sub ^R }} :- var R.

% replace 1 and 0 by their ring projectors
pred ring.notations i:preterm, o:preterm.
ring.notations (varp \"1\" _)  {{:raw ring_1 ^R }}.
ring.notations (varp \"0\" _)  {{:raw ring_0 ^R }}.
ring.notations (varp \"--\" _) {{:raw ring_neg ^R }}.
ring.notations (varp \"+\" _) {{:raw ring_add ^R }}.
ring.notations (varp \"*\" _) {{:raw ring_mul ^R }}.
ring.notations (varp \"-\" _) {{:raw ring_sub ^R }}.

ring.notations (varp _ _ as V) V.
ring.notations (combp A B) (combp A1 B1) :-
  ring.notations A A1,
  ring.notations B B1.
ring.notations (absp A B) (absp A1 B1) :-
  ring.notations A A1,
  ring.notations B B1.
ring.notations (typing A T) (typing A1 T) :-
  ring.notations A A1.

:before \"infer:app\"
infer {{:raw ^A IN ^B }} R T :-
  B = varp _ _,
  infer B _ BT,
  BT = (ptycon \"ring\" [CARRIER_TYPE]), !,
  BC = {{:raw ring_carrier ^B }},
  ring.notations A A1,
  A2 = typing A1 CARRIER_TYPE,
  is_ring B CARRIER_TYPE => infer {{:raw ^A2 IN ^BC }} R T.

:before \"infer:app\"
infer (combp _ RING as OP) R T :- ring_op OP, not(var T), !, is_ring RING T,
  infer OP R T.

:before \"infer:app\"
infer (combp _ RING as OP) R T :- ring_op OP, var T, !, print \"DANGER\", is_ring RING T,
  infer OP R T.

");;
Hol_elpi.typecheck_current ();;
